...
class World:
        ...
        while x < self.world_width:
            if gaps >= 2:
                ...
                if random.random() < 0.2:
                    ...
                    # Reduced jump/gap parameters:
                    max_jump = 100                     # was 140
                    max_horiz_tiles = int(self.tile_width * 1.3)  # was 1.5×tile_width

                    # Vertical offset
                    v = random.randint(-max_jump, max_jump)

                    # Limit horizontal based on vertical
                    horiz_max = max_horiz_tiles
                    if v != 0:
                        allowed = (horiz_max**2 - v**2)**0.5
                        horiz_max = int(max(94, min(allowed, horiz_max)))

                    # Pick a gap between one tile and our reduced max
                    h = random.randint(94, horiz_max)

                    # Clamp next platform height
    ...
    def _make_segment(self, x, y):
        ...
        # 30% chance to spawn one vertical pillar (pine tree)
    ...
    def draw(self, screen, camera_x):
        ...
        for sw in self.switches:
            col = (0, 0, 255) if self.current_era == "past" else (255, 165, 0)
